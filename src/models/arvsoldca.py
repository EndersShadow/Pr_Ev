#Comparable Analysis: After Renovations Value (ARV) Sold
#comparable.
...
class ARVSold(object):
    ...
    def save_to_mongo(self):
        Database.insert(collection= 'arvsoldca', data = self.json())
    ...
    def from_mongo(cls, id):
        arvsoldca_data = Database.find_one(collection='arvsoldca',
                                      query={'_id:id]'})
        return cls(**arvsoldca_data)

    @staticmethod
    def from_mongo(id):
        return Database.find_one(collection='arvsoldca',
                                 query={'_id': id})